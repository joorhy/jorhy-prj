...
int CJorFileHelper::OpenFile(J_OS::CTCPSocket *recvSocket, const char *pResid, j_time_t begin, j_time_t end)
{
	...
	CXJoSdk::Instance()->MakeReqHeader(temp_buff, NULL, jo_open_file_req, jo_intact_pack, 0, sizeof(J_VodPlayData));
...
{
	char temp_buff[2048] = {0};
	CXJoSdk::Instance()->MakeReqHeader(temp_buff, NULL, jo_req_data_req, jo_intact_pack, 0, sizeof(J_RequestData));
