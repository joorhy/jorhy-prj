...
CJorFileReader::CJorFileReader(const char *pResid)
{
	...
	//m_timer.Create(TIMER_INTERVAL, CJorFileReader::TimerThread, (unsigned long)this);
	J_OS::LOGINFO("CJorFileReader::CJorFileReader");
}
...
int CJorFileReader::ListRecord(uint64_t beginTime, uint64_t endTime)
{
	...
	{
	    J_OS::LOGINFO("CJorFileReader::ListRecord GetHistoryFile Error");
	    return J_DB_ERROR;
	}
	m_fileVec.clear();
...
int CJorFileReader::OpenFile()
{
	...
	{
		J_OS::LOGERROR("CJorFileReader::OpenFile() file not found, filename = %s", fileName);
		return J_FILE_ERROR;
	}
	...
	if (m_pFileId == NULL)
	{
		J_OS::LOGERROR("CJorFileReader::OpenFile() fopen error");
		return J_FILE_ERROR;
	}
	...
	{
		J_OS::LOGINFO("CJorFileReader::OpenFile() body File Broken");
	...
	{
		J_OS::LOGINFO("CJorFileReader::OpenFileByName head File Broken");
		return J_FILE_BROKEN;
	}
...
int CJorFileReader::CalcPosition(uint64_t timeStamp, uint32_t interval)
{
	int nRet = J_OK;
	nRet = OpenFile();
	if (nRet != J_OK)
	...
	{
	    J_OS::LOGINFO("CJorFileReader::CalcPosition J_OUT_RANGE");
		return J_OUT_RANGE;
	}
	...
	{
		J_OS::LOGINFO("CJorFileReader::CalcPosition J_NO_RESOURCE");
