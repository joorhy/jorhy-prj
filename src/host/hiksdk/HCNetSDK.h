...
#define NET_ITS_GET_CUSTOMINFO_CFG				5097  //获取自定义信息
#define NET_ITS_SET_CUSTOMINFO_CFG				5098  //设置自定义信息
...
typedef struct 
{
	...
	DWORD dwVideoFrameRate;	//帧率 0-全部; 1-1/16; 2-1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6; 9-8; 10-10; 11-12; 12-16; 13-20; V2.0版本中新加14-15; 15-18; 16-22;
	                        //17-25；18-30；19-35；20-40；21-45；22-50；23-55；24-60
	...
	BYTE  byIntervalBPFrame;//0-BBP帧; 1-BP帧; 2-单P帧(2006-08-11 增加单P帧的配置接口，可以改善实时流延时问题)
 	...
 	BYTE  byAudioEncType;   //音频编码类型 0-OggVorbis;1-G711_U;2-G711_A;5-MP2L2;6-G276;7-AAC
	BYTE  byVideoEncComplexity; //视频编码复杂度，0-低，1-中，2高
	BYTE  byEnableSvc; //0 - 不启用SVC功能；1- 启用SVC功能
    BYTE  byres[8];//这里保留音频的压缩参数
...
typedef struct 
{
	...
	BYTE byRecordType;	//0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警, 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-所有报警,14-智能交通事件，15保留，16区域入侵侦测，17音频异常侦测
	char reservedData[3];
...
typedef struct 
{
	DWORD dwAlarmType;/*0-信号量报警,1-硬盘满,2-信号丢失,3－移动侦测,4－硬盘未格式化,5-读写硬盘出错,6-遮挡报警,7-制式不匹配, 8-非法访问, 9-视频信号异常,
                        10-录像异常,11- 智能场景变化,12-阵列异常,13-前端/录像分辨率不匹配,14-申请解码资源失败*/
...
typedef struct tagNET_VCA_GET_UP
{
	...
	BYTE    byMode;  //起身检测模式，0-大床通铺模式，1-高低铺模式
    BYTE	byRes[5];		// 保留字节
...
typedef struct tagNET_DVR_PLATE_RESULT
{
    ...
    BYTE	 byVehicleType;     //0-客车、1-货车、2-轿车、3-面包车、4-小货车
...
typedef struct tagNET_DVR_FIND_PICTURE_PARAM
{
    DWORD  dwSize;         // 结构体大小 
    LONG   lChannel;       // 通道号
    BYTE   byFileType;     // 查找的图片类型:0定时抓图1 移动侦测抓图 2 报警抓图3  报警 | 移动侦测抓图 4 报警 & 移动侦测抓图 6 手动抓图 ,9-智能图片,10- PIR报警，11- 无线报警，12- 呼救报警,，0xa 预览时截图, 0xd 人脸侦测,0xe 保留，Oxf 入侵区域侦测，0xff全部类型
...
typedef enum 
{
	...
	NET_SDK_DOWNLOAD_IPC_CFG_FILE	//下载IPC配置文件
...
typedef struct tagNET_DVR_ALARMHOST_PRINTER_CFG
{
	...
	DWORD	dwDeviceInfo;			//设备信息，按位表示。bit0-交流电断电、bit1-交流电断电恢复，bit2-蓄电池欠压、bit3-蓄电池欠压恢复，bit4-电话线断线、bit5-电话线断线恢复，bit6-测试报告、bit7-防拆、bit8-防拆恢复、bit9-485设备掉线、bit10-485设备掉线恢复、bit11-无线网络异常、bit12-无线网络恢复正常、bit13-有线网络异常、bit14-有线网络恢复正常、bit15-扩展总线异常、bit17-扩展总线恢复正常
...
typedef struct tagNET_ITC_PLATE_RECOG_PARAM
{
    ...
    DWORD dwRecogMode;  /*识别的类型，
		...
		bit6-帧识别或场识别：0-帧识别，1-场识别； 
		bit7-晚上或白天：0-白天，1-晚上 
	bit8-微小车牌：0-不启用，1-启用微小车牌识别(像素60～80)*/
...
typedef struct tagNET_ITC_LANE_HVT_PARAM
{
	...
	NET_ITC_INTERVAL_PARAM struInterval; //抓拍间隔参数
	NET_ITC_RADAR_PARAM struRadar;	//雷达配置 
...
{
	DWORD  dwSize;
	BYTE   byAlarmType;/* 报警类型,1-设备上线，2-设备离线 3-服务器上线 4-服务器离线 5-管理员登陆 6-预案开始,7-预案结束*/
	BYTE   byDeviceType;/* 设备类型(只能获取) */ 
	BYTE   byRes1[2];
	DWORD  dwDeviceIndex;//设备序号
	BYTE   byRes2[92];
...
typedef struct tagNET_ITS_PLATE_RESULT
{
	...
	DWORD   dwSnapFirstPicTime;//端点时间(ms)（抓拍第一张图片的时间）
...
typedef struct tagNET_DVR_FTPCFG_V40
{
	...
	BYTE 				bySubDirMode;       /*二级目录，0x1=使用通道名,0x2=使用通道号
											   0x3=使用时间(年月日),0x4=使用车道号 0x5-使用自定义,0x6=违规类型,0x7=方向,0x8=地点*/
...
/*************************************PJ03C2013060554停车场DS-TP5016-P begin*****************************/
//外接灯
...
typedef struct tagNET_DVR_LAMP_CTRL_INFO
{
	...
	BYTE      byCtrlChannelIndex;/*交替控制通道号*/
	BYTE      byRes[2];
...
/*************************************PJ03C2013060554停车场DS-TP5016-P end*******************************/
...
typedef struct  tagNET_DVR_ALARMHOST_SENSOR_JOINT_CFG
{
	...
	BYTE		bySensorAlarmTypeJointAlarmOut[ALARMHOST_ALARMOUT_NUM/*64*/];//模拟量按类型联动触发器，该字段是bySensorJointAlarmOut的扩展。数组下标表示触发器号，数组元素的值按位表示报警类型，各个类型间是"或"的关系。类型值定义：1-限1；2-限2；3-限3；4-限4；5-故障
	BYTE		bySesorAlarmTypeJointSiren[ALARMHOST_MAX_SIREN_NUM]; //模拟量按类型联动警号，该字段是bySensorJointSiren的扩展。数组下标表示警号序号，数组元素的值按位表示报警类型，各个类型间是"或"的关系。类型值定义：1-限1；2-限2；3-限3；4-限4；5-故障
	BYTE		byRes[56];
...
/************************视频质量诊断分析仪 iVMS-6200(D) V2.1.0 begin***********/
//平台查询诊断服务器的状态
...
/************************视频质量诊断分析仪 iVMS-6200(D) V2.1.0 end***********/
...
typedef struct tagNET_DVR_PREVIEW_SWITCH_CFG
{
	...
	BYTE	byPreviewNumber;/*预览数目,0-1画面,1-4画面,2-9画面,3-16画面, 4-6画面, 5-8画面
								0xff:最大画面*/
	BYTE	byEnableAudio;//是否声音预览,0-不预览,1-预览
...
typedef union tagNET_DVR_VD_UNION
{
	...
	NET_DVR_OPERATE_VD_PARAM	struHikVDParam;		// 海康虚拟磁盘
...
{
	BYTE				byVDType;	//虚拟磁盘类型：0-海康虚拟磁盘 1-逻辑卷 2-DVR网盘
	BYTE				byRes1[3];
...
typedef enum tagHIKVQD_TYPE
{
    HIKVQD_BLUR_FLAG     = 0x00000001,
		HIKVQD_LUMA_FLAG     = 0x00000002,
		HIKVQD_CHROMA_FLAG   = 0x00000004,
		HIKVQD_SNOW_FLAG     = 0x00000008,
		HIKVQD_STREAK_FLAG   = 0x00000010,
		HIKVQD_FREEZE_FLAG   = 0x00000020,
		HIKVQD_SIGLOSE_FLAG  = 0x00000040,
		HIKVQD_PTZ_FLAG      = 0x00000080
}HIKVQD_TYPE;
