...
void RTSPClient::handleRequestError(RequestRecord* request) {
  ...
  if (request->handler() != NULL) (*request->handler())(this, resultCode, strDup(envir().getResultMsg()), pActionRtspClient);
}
...
void RTSPClient::responseHandlerForHTTP_GET(RTSPClient* rtspClient, int responseCode, char* responseString, void * ActionRtspClient) {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
	...
	*responseEnd = saved;
	...
	(*foundRequest->handler())(this, resultCode, resultString, pActionRtspClient);
      } else {
