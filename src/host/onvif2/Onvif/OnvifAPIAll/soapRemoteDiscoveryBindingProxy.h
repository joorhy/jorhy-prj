...
#include "soapH.h"
class RemoteDiscoveryBinding
{   public:
	/// Runtime engine context allocated in constructor
	struct soap *soap;
	/// Endpoint URL of service 'RemoteDiscoveryBinding' (change as needed)
	const char *endpoint;
	...
	{ soap = soap_new(); endpoint = "http://localhost:80"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	virtual ~RemoteDiscoveryBinding() { if (soap) { soap_destroy(soap); soap_end(soap); soap_free(soap); } };
	/// Invoke 'Hello' of service 'RemoteDiscoveryBinding' and return error code (or SOAP_OK)
	virtual int __ns18__Hello(ns6__HelloType *dn__Hello, ns6__ResolveType *dn__HelloResponse) { return soap ? soap_call___ns18__Hello(soap, endpoint, NULL, dn__Hello, dn__HelloResponse) : SOAP_EOM; };
	/// Invoke 'Bye' of service 'RemoteDiscoveryBinding' and return error code (or SOAP_OK)
	virtual int __ns18__Bye(ns6__ByeType *dn__Bye, ns6__ResolveType *dn__ByeResponse) { return soap ? soap_call___ns18__Bye(soap, endpoint, NULL, dn__Bye, dn__ByeResponse) : SOAP_EOM; };
};
#endif
