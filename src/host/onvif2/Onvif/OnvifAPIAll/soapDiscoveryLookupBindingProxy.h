...
#include "soapH.h"
class DiscoveryLookupBinding
{   public:
	/// Runtime engine context allocated in constructor
	struct soap *soap;
	/// Endpoint URL of service 'DiscoveryLookupBinding' (change as needed)
	const char *endpoint;
	...
	{ soap = soap_new(); endpoint = "http://localhost:80"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	virtual ~DiscoveryLookupBinding() { if (soap) { soap_destroy(soap); soap_end(soap); soap_free(soap); } };
	/// Invoke 'Probe' of service 'DiscoveryLookupBinding' and return error code (or SOAP_OK)
	virtual int __ns19__Probe(ns6__ProbeType *dn__Probe, ns6__ProbeMatchesType *dn__ProbeResponse) { return soap ? soap_call___ns19__Probe(soap, endpoint, NULL, dn__Probe, dn__ProbeResponse) : SOAP_EOM; };
};
#endif
