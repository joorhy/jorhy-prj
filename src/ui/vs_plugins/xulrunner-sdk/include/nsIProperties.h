/*
 * DO NOT EDIT.  THIS FILE IS GENERATED FROM e:/builds/moz2_slave/rel-m-rel-xr_w32_bld-000000000/build/xpcom/ds/nsIProperties.idl
 */
...
#define NS_DECL_NSIPROPERTIES \
  ...
  NS_IMETHOD Has(const char * prop, bool *_retval); \
  NS_IMETHOD Undefine(const char * prop); \
  NS_IMETHOD GetKeys(uint32_t *count, char * **keys); 
...
#define NS_FORWARD_NSIPROPERTIES(_to) \
  ...
  NS_IMETHOD Has(const char * prop, bool *_retval) { return _to Has(prop, _retval); } \
  NS_IMETHOD Undefine(const char * prop) { return _to Undefine(prop); } \
  NS_IMETHOD GetKeys(uint32_t *count, char * **keys) { return _to GetKeys(count, keys); } 
...
#define NS_FORWARD_SAFE_NSIPROPERTIES(_to) \
  ...
  NS_IMETHOD Has(const char * prop, bool *_retval) { return !_to ? NS_ERROR_NULL_POINTER : _to->Has(prop, _retval); } \
  NS_IMETHOD Undefine(const char * prop) { return !_to ? NS_ERROR_NULL_POINTER : _to->Undefine(prop); } \
  NS_IMETHOD GetKeys(uint32_t *count, char * **keys) { return !_to ? NS_ERROR_NULL_POINTER : _to->GetKeys(count, keys); } 
