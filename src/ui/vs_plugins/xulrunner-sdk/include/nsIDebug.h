/*
 * DO NOT EDIT.  THIS FILE IS GENERATED FROM e:/builds/moz2_slave/rel-m-rel-xr_w32_bld-000000000/build/xpcom/base/nsIDebug.idl
 */
...
#define NS_DECL_NSIDEBUG \
  NS_IMETHOD Assertion(const char * aStr, const char * aExpr, const char * aFile, int32_t aLine); \
  NS_IMETHOD Warning(const char * aStr, const char * aFile, int32_t aLine); \
  NS_IMETHOD Break(const char * aFile, int32_t aLine); \
  NS_IMETHOD Abort(const char * aFile, int32_t aLine); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIDEBUG(_to) \
  NS_IMETHOD Assertion(const char * aStr, const char * aExpr, const char * aFile, int32_t aLine) { return _to Assertion(aStr, aExpr, aFile, aLine); } \
  NS_IMETHOD Warning(const char * aStr, const char * aFile, int32_t aLine) { return _to Warning(aStr, aFile, aLine); } \
  NS_IMETHOD Break(const char * aFile, int32_t aLine) { return _to Break(aFile, aLine); } \
  NS_IMETHOD Abort(const char * aFile, int32_t aLine) { return _to Abort(aFile, aLine); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIDEBUG(_to) \
  NS_IMETHOD Assertion(const char * aStr, const char * aExpr, const char * aFile, int32_t aLine) { return !_to ? NS_ERROR_NULL_POINTER : _to->Assertion(aStr, aExpr, aFile, aLine); } \
  NS_IMETHOD Warning(const char * aStr, const char * aFile, int32_t aLine) { return !_to ? NS_ERROR_NULL_POINTER : _to->Warning(aStr, aFile, aLine); } \
  NS_IMETHOD Break(const char * aFile, int32_t aLine) { return !_to ? NS_ERROR_NULL_POINTER : _to->Break(aFile, aLine); } \
  NS_IMETHOD Abort(const char * aFile, int32_t aLine) { return !_to ? NS_ERROR_NULL_POINTER : _to->Abort(aFile, aLine); } 
