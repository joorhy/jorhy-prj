...
int libvlc_video_toggle_record( libvlc_media_player_t *p_mi,
							   const char *psz_filepath, const char *psz_fileame );
...
*/
LIBVLC_API
char *libvlc_var_get_string(libvlc_media_player_t *p_mi, const char *key);
...
*
* \param p_mi media player
* \param channel 通道号,default 0;
* \param position 显示位置：LEFT = 0x1,RIGHT = 0x2,TOP = 0x4,BOTTOM = 0x8
* \param duration 持续时间，以纳秒为单位
* \param psz_osd 要显示的文本，必须是以空字符为结尾的UTF-8字串
* \return 0 on success, -1 on error
* \version LibVLC 1.1.1 or later
*/
LIBVLC_API int libvlc_video_set_osd(libvlc_media_player_t *p_mi,int channel,int position,int64_t duration,char *psz_osd);

LIBVLC_API char *libvlc_video_get_vout_string(libvlc_media_player_t *p_mi,const char *psz_key);

LIBVLC_API void libvlc_video_set_callbacks_custom( libvlc_media_player_t *mp,
												  void(*display)(void *user,void *picture),
												  void *data);

LIBVLC_API void *libvlc_media_player_get_input( libvlc_media_player_t *p_mi );

LIBVLC_API void libvlc_media_player_release_input( void *p_input );
