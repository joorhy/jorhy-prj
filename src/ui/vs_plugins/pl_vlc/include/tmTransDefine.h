...
#define PTZ_LOCK_KEEPWATCH		35	//锁定看守位，内部不再调用自动看守位，调用PTZ_UNLOCK_KEEPWATCH或连接断开回复
#define PTZ_UNLOCK_KEEPWATCH	36	//解锁看守位，回复原来自动状态
...
typedef struct tmPtzSchedCfg_t
{
	...
	unsigned char		byLimitSpeed[64];							//Zoom的每倍显示速度0-是1倍，63-64倍,当数组值为0是不限制
...
typedef struct tmToManagerImageInfo_t
{
	...
	unsigned char		byImageMode;					//抓图的模式0-报警联动抓图，1-移动联动抓图，2-手动抓图
...
{	
	...
	unsigned char		byControlMode;					//控制模式0-角度，1-相对步进
}tmPtzParameter_t;
...
typedef struct tmPtzIntegrateCfg_t
{
	...
	unsigned int				dwVerticallyStation;				//垂直位置绝对角度*1000
	unsigned int				dwHorizontallyStation;				//水平位置绝对角度*1000
	unsigned int				dwVerticallyVisualAngle;			//当前可视垂直角度*1000
	unsigned int				dwHorizontallyVisualAngle;			//当前可视水平角度*1000
	unsigned int				dwZoomValue;						//当前的变倍数*1000
	unsigned char				byControlMode;						//控制模式0-角度，1-相对步进
	unsigned char				byTemp[3];
}tmPtzIntegrateCfg_t;
...
typedef struct tmVideoInCfg_t
{
	...
	//AGC增益0=42dB,32=36dB,64=30dB,96=24dB,128=18dB,160=12dB,192=6dB,224-自动
	unsigned char               byAgc;	
...
{
	unsigned int			dwSize;									//本结构大小
	unsigned int			dwLongitude;							//经度*10000
	unsigned int			dwLatitude;								//纬度*10000
}tmTheodoliteCfg_t;
