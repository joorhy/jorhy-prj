...
//By YuanJJ
typedef void (*libvlc_video_cbk_custom)(void *opaque);
                                        ...
                                        subpicture_t *subpicture)
{
	...
	//BY yuanJJ
	libvlc_video_cbk_custom callback = var_InheritAddress(vd,"yuan-vout-callback");
	if(callback)
	{
		void *usrdata	= var_InheritAddress(vd,"yuan-vout-data");
		callback(usrdata);
	}
