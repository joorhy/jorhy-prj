...
CAdapterManager::CAdapterManager(int)
{
...
int CAdapterManager::StopVoice(const char *pResId, const j_socket_t nSocket)
{
	...
	J_ChannelStream *pVoiceChannel = static_cast<J_ChannelStream *>(CAdapterFactory::Instance()->GetInstance(pResId, OBJ_CHANNEL, key.stream_type));
...
{
    int nStreamType = 0;
	J_StreamParser *pVideoParser = static_cast<J_StreamParser *>(CAdapterFactory::Instance()->GetInstance(pResId, OBJ_CHANNEL, nStreamType));
